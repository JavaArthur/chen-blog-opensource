...
DB_NAME="${CF_D1_NAME:-channing-blog-db}"
R2_NAME="${CF_R2_NAME:-channing-blog-images}"
KV_NAME="${CF_KV_NAME:-channing-blog-cache}"
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: npm run cf:init -- [--with-kv] [--site-url=https://your-domain.com] [--db-name=channing-blog-db] [--r2-name=channing-blog-images] [--kv-name=channing-blog-cache]" >&2
      exit 1
      ;;
  esac
done
