...
- 首页身份、作品、实验室与联系信息由现有 D1 `site_settings` 管理；新增受保护的 `/admin/studio` 管理入口。文章继续由 `posts` 表和既有 `/admin/posts`、`/editor` 工作流管理。
...
公开导航默认是：作品、文章、实验室、关于我、GitHub、RSS、搜索，并继续允许在后台排序和维护链接。后台入口继续可用，但从公开导航移除；剪报和工具收藏只在文章/实验室相关上下文提供入口，不再占据首屏导航。
...
第一期不引入 D1 schema 变更，但全部个人主站内容均可后台维护。复用 `site_settings` 存储以下 JSON 配置：

- `studio_profile`：身份文案、主张、联系链接与能力卡。
- `studio_projects`：作品名称、摘要、技术标签、链接、排序和显示状态。
- `studio_labs`：实验室项目名称、说明、链接、分类和排序。
- `studio_featured_posts`：可选的精选文章 slug 列表；为空时首页自动读取最新 3 篇原创文章。

新增类型化解析模块，非法 JSON 或缺字段时回退到安全的空列表/默认文案。文章仍由现有 `posts` 仓储查询，工具收藏仍读 `tools` 表。

新增 `/admin/studio` 管理页，分区编辑身份、能力、作品、实验室和精选文章；保存调用已有管理员鉴权的 `/api/admin/settings`。这样无需新增 schema，却能在后台动态发布与调整个人主站内容。后续若作品规模超出单个设置项可维护范围，再评估迁入独立表。
...
- 新建 `app/work/page.tsx`、`app/labs/page.tsx`、`app/about/page.tsx`，读取类型化的 Studio settings 与 `SiteHeader` / `SiteFooter`。
- 新建聚焦的 Studio 展示组件，避免把新首页塞进现有主题组件；现有多主题文章阅读能力保持不变。
- `SiteHeader` 接受页面上下文，公开页使用稳定的 Studio 导航；不依赖数据库内可能过时的默认链接作为兜底。
- 新建 `app/admin/(protected)/studio/page.tsx` 与对应客户端表单，复用现有 `site_settings` 和管理员 settings API，不改变文章编辑器或文章管理页。
...
- 管理员可在 `/admin/studio` 修改身份、作品、实验室和精选文章；刷新公开页面后能读取新值，非法配置不会导致公开页报错。
