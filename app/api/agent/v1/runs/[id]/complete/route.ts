...
import { parseCompleteRunPayload } from '@/lib/agent-knowledge/validation'
...
export async function POST(
  ...
  let input
  try {
    input = parseCompleteRunPayload(await parseJsonBody(req))
  } catch (error) {
  ...
  try {
    ...
    const result = await completeAgentRun(route.db, id, input)
