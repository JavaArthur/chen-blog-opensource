...
把任何 Markdown 或网页内容，以"和后台编辑器一模一样"的效果，发布到你自己的 Channing Blog。
...
```
/channing-blog-publish path/to/article.md
/channing-blog-publish https://example.com/article
/channing-blog-publish                             # 然后粘贴 Markdown
```

### 带标志

```
/channing-blog-publish ~/note.md --status=published --category=AI --yes
/channing-blog-publish ~/note.md --dry-run --json  # 预检不发
/channing-blog-publish ~/note.md --json            # 机器可读输出
```
...
1. 命令行 `--api-url=... --token=...`
2. 环境变量 `CHANNING_BLOG_API_URL` / `CHANNING_BLOG_API_TOKEN`
3. 环境变量 `QMBLOG_API_TOKEN`（兼容旧名）
4. `~/.claude/skills/channing-blog-publish/config.json`
...
```bash
# 读配置
API_URL="${CHANNING_BLOG_API_URL:-$(jq -r .apiUrl ~/.claude/skills/channing-blog-publish/config.json 2>/dev/null)}"
TOKEN="${CHANNING_BLOG_API_TOKEN:-${QMBLOG_API_TOKEN:-$(jq -r .token ~/.claude/skills/channing-blog-publish/config.json 2>/dev/null)}}"
...
标签：AI, 工作流, 自动化
slug：2026-04-28-five-agents
Edit：https://note.aichanning.cn/editor?edit=2026-04-28-five-agents
View：https://note.aichanning.cn/2026-04-28-five-agents
...
```json
{
  ...
  "id": 42,
  "status": "draft",
  "editUrl": "https://note.aichanning.cn/editor?edit=2026-04-28-five-agents",
  "viewUrl": "https://note.aichanning.cn/2026-04-28-five-agents",
  "uploads": {
    "localImages": 3,
    "remoteRehosted": 1,
    "remoteFailed": [
...
任何 agent 在自己 prompt 里读到 `ecosystem/channing-blog-publish-skill/SKILL.md` 就知道字段、接口、错误码。典型调用：

```
Agent X（写完稿子后）→ 触发 /channing-blog-publish ~/drafts/foo.md --yes --status=draft --json
...
- name: Publish as draft
  env:
    CHANNING_BLOG_API_URL: https://note.aichanning.cn
    CHANNING_BLOG_API_TOKEN: ${{ secrets.CHANNING_BLOG_TOKEN }}
  run: |
    # 这里不走 skill，直接调后端（skill 只是文档；真要 CI 建议抽成独立 npm CLI）
    curl -fsSL -X POST "$CHANNING_BLOG_API_URL/api/posts" \
      -H "Authorization: Bearer $CHANNING_BLOG_API_TOKEN" \
      -H "Content-Type: application/json" \
      -d @payload.json
```
...
- **channing-blog-publish (本 skill)**：命令行 / 任意 AI agent 入口
